...
To illustrate, if the quantity of interest is :py:data:`'output_2'`, then the item in the dictionary is configured as::
...
The sparse PCE is selected with :py:data:`'sparse'`. It starts from the
same full candidate basis, but builds a reduced basis from the available
samples with the stepwise regression method described in :ref:`lab:pce`
and Abraham et al. :cite:`Abraham2017`. This is useful when the full basis
would require too many model evaluations. For example::
...
The value of :py:data:`'n samples'` is a user-defined budget. RHEIA does not
increase it automatically until a target error is reached. After fitting the
sparse basis, RHEIA reports the LOO error in the result summary. Treat this
error as a diagnostic and repeat the run with a larger sample budget or
polynomial order when the LOO error, moments or Sobol' indices are not stable.

.. tip::

   For a sparse PCE, start with enough samples to leave clear degrees of
   freedom for least-squares fitting. Then increase :py:data:`'n samples'`
   until the reported LOO error and the quantities used in your decision no
   longer change materially.

'sampling method': sampling_method
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For the construction of a PCE, a number of model evaluations are required. The samples for model evaluation can be generated
...
In addition to the statistical moments, the data for generating the probability density function (pdf) and cumulative distribution function (cdf) can be generated.
...
When combining the examples in the previous section, a configured uncertainty quantification dictionary with the necessary items looks as follows:
