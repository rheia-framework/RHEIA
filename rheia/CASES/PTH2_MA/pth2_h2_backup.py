...
class Evaluation:
    ...
    def p_demand_nominal(self):
        ...
        lhv_h2 = 33.33 * 1e-9 # TWh/kg
        
        # hydrogen per hour required
        h_to_supply_demand = 8760.
        m_h2 = self.par['demand_TWh']/h_to_supply_demand/lhv_h2 # kg/h
        
        self.par['n_liq'] = m_h2 #kg/h
        
        p_pem_nom = self.par['e_pem'] * m_h2 * 1e3 #Wh
        p_desal_nom = self.par['e_desal'] * m_h2 * self.par['h2o_to_h2_ratio_pem'] * 1e3 #Wh

        e_comp_liq = self.compressor_liq(1.) #Wh/kg_h2
        e_comp_liq *= m_h2 * 0.0044
        self.par['e_comp_liq'] = e_comp_liq/1e3

        p_liq_nom = self.par['e_liq'] * m_h2 * 1e3 
        
        p_store_h2_nom = 0. #self.par['e_h2_compr'] * m_h2 * 1e3
        
        return p_pem_nom, p_desal_nom, p_liq_nom, p_store_h2_nom
    ...
    def discharge_bat(self, p_bat):
        ...
        self.SOCarray[self.t] = self.e_bat[self.t] / self.par['n_bat']
    ...
    def evaluation(self):
        ...
        p_pem_nom, p_desal_nom, p_liq_nom, p_store_h2_nom = self.p_demand_nominal()

        self.m_h2 = np.zeros(len(self.sol_irr)) # * self.m_h2_max
        self.p_to_pemel = np.zeros(len(self.sol_irr)) # * self.m_h2_max
        self.e_bat = np.ones(len(self.sol_irr)) * self.par['n_bat']
        self.SOCarray = np.ones(len(self.sol_irr))        
        self.p_desal = np.zeros(len(self.sol_irr))
        
        checker = []
        
        self.res['p_dem_nom'] = p_pem_nom + p_desal_nom + p_liq_nom
        ...
        # no renewable power 
        p_division = p_pem_nom + p_store_h2_nom + p_desal_nom + p_liq_nom / self.par['eff_bat']**2. 
        p_pem_frac = (p_pem_nom + p_store_h2_nom)/p_division
        p_desal_frac = p_desal_nom/p_division
        p_liq_frac = p_liq_nom / (self.par['eff_bat']**2.) /p_division
        ...
        p_division_2 = p_pem_nom + p_desal_nom + p_liq_nom 
        p_pem_frac_2 = p_pem_nom/p_division_2
        p_desal_frac_2 = p_desal_nom/p_division_2
        p_liq_frac_2 = p_liq_nom /p_division_2
        ...
        for t, p_ren in enumerate(self.res['p_renew']):
            ...
            if p_dem > 0.:
                ...
                self.p_to_pemel[self.t] = p_pem_nom + p_pemel_applied
                ...
                self.charge_bat(p_dem * p_liq_frac + 
                ...
                # supply remaining power for liquefaction
                p_liq_still_needed = p_liq_nom - self.res['p_renew'][self.t] * p_liq_frac_2
                                
                fail, missing = self.discharge_bat(p_pem_needed + p_desal_needed + p_liq_still_needed)
                checker.append(fail)
                

                if fail:
                    h2_needed = missing / self.par['eff_fc'] #Wh of h2 needed
                    n_fc = missing
                    if n_fc > self.par['n_fc']:
                        self.par['n_fc'] = n_fc
                        
                    self.res['h2_needed'] += h2_needed
        ...
        if n_pauses * self.par['delay_restart'] > 0 or self.res['h2_needed']/1e12 > self.par['demand_TWh']:
    ...
    def lifetime(self):
        ...
        #self.lifeBat = self.battery_lifetime(self.e_bat/self.par['n_bat'])
    ...
    def cost(self):
        ...
        fc_cost = self.par['n_fc'] * (crf * self.par['capex_fc'] +
                                      self.par['opex_fc']* self.par['capex_fc'])
        components_cost += fc_cost
        ...
        boat_cap_m3 = boat_cap_twh * 1e6 * 3600. / 120. / 70.85 #m3    
        
        boat_cap_tonne = boat_cap_twh * 1e6 * 3600. / 120. /1e3 #tonne
        ...
        ship_average_power_main_engine =  13500e3 * 336./8760. * time_at_sea / 0.77
        ship_average_power_main_engine += 11475e3 * 2016./8760. * time_at_sea / 0.77
        ship_average_power_main_engine += 8775e3 * 3024./8760. * time_at_sea / 0.77
        ship_average_power_main_engine += 6750e3 * 336./8760. * time_at_sea / 0.77
        ship_average_power_main_engine += 4725e3 * 336./8760. * time_at_sea / 0.77
        ship_average_power_main_engine += 3375e3 * 336./8760. * time_at_sea / 0.77
        ship_average_power_main_engine += 1350e3 * 236./8760. * time_at_sea / 0.77 
        ship_average_power_main_engine += 675e3 * 100./8760. * time_at_sea / 0.77 
        ship_average_power_main_engine += 0. * 2040./8760. * time_at_poarts / 0.23
        
        h2_produced = self.par['demand_TWh']*1e6

        h2_produced -= self.res['h2_needed']/1e6  

        boat_cap_t_ref = 10000. #tonne
        ...
        bog_fuel = (1. - (1.-self.par['bog'])**(time_trip/24.)) * h2_produced #MWh boil-off per year. only 1 part of the trip, het naartoe gaan, is considered as fuel

        self.mwh_produced = h2_produced - fuel_needed
        ...
        if fuel_needed < bog_fuel:
            ...
            h2_needed_for_refr /= (33.33 * self.par['eff_fc']) #kWh power/ (kWh power per kg in H2) = kg H2 needed
            h2_needed_for_refr *= 33.33e-3 #MWh H2 needed
        ...
        if self.res['h2_needed']/1e12 > 0.10:
            self.final_check = False

        # the levelized cost of hydrogen
        cost =  components_cost + cost_boat
        extra_backup = cost / (1. - self.res['h2_needed']/1e12 - self.fuel_needed/1e6 - self.h2_needed_for_refr/1e6) - cost
        cost += extra_backup
    ...
    def calc_lca(self):
        ...
        n_liq = self.par['n_liq'] / 1e3 * 24. / (self.par['life_liq']/self.par['life_sys'])
        n_refr = self.refr_kg_h / 1e3 * 24. / (self.par['life_liq']/self.par['life_sys'])        
        ...
        for index,method in enumerate(self.pb_names):
            ...
            self.res[method] += self.dict_lcia_pb['pblcia_h2liq_SSP1-Base_reference'][index] * n_liq
            self.res[method] += self.dict_lcia_pb['pblcia_h2liq_SSP1-Base_reference'][index] * n_refr
            self.res[method] += self.dict_lcia_pb['pblcia_ship_SSP1-Base_reference'][index] * n_ship 
            self.res[method] += self.dict_lcia_pb['pblcia_desal_SSP1-Base_reference'][index] * n_desal 
            self.res[method] += self.dict_lcia_pb['pblcia_pem_SSP1-Base_reference'][index] * n_fc 
            extra_backup = self.res[method] / (1. - self.res['h2_needed']/1e12 - self.fuel_needed/1e6 - self.h2_needed_for_refr/1e6) - self.res[method]
            ...
            self.res['%s_frac_h2liq' %method] = self.dict_lcia_pb['pblcia_h2liq_SSP1-Base_reference'][index] * n_liq / self.res[method]
            self.res['%s_frac_refr' %method] = self.dict_lcia_pb['pblcia_h2liq_SSP1-Base_reference'][index] * n_refr / self.res[method]
            self.res['%s_frac_ship' %method] = self.dict_lcia_pb['pblcia_ship_SSP1-Base_reference'][index] * n_ship / self.res[method]
            self.res['%s_frac_desal' %method] = self.dict_lcia_pb['pblcia_desal_SSP1-Base_reference'][index] * n_desal / self.res[method]
            self.res['%s_frac_backup' %method] = self.dict_lcia_pb['pblcia_pem_SSP1-Base_reference'][index] * n_fc / self.res[method] + extra_backup / self.res[method]
        ...
        if self.all_dbs:
            ...
            for db in self.dbs:
                ...
                for index,method in enumerate(self.pb_names):
                    ...
                    self.res[method] += self.dict_lcia_pb['pblcia_h2liq' + db.split('remind')[1]][index] * n_liq
                    self.res[method] += self.dict_lcia_pb['pblcia_h2liq' + db.split('remind')[1]][index] * n_refr
                    self.res[method] += self.dict_lcia_pb['pblcia_ship' + db.split('remind')[1]][index] * n_ship 
                    self.res[method] += self.dict_lcia_pb['pblcia_desal' + db.split('remind')[1]][index] * n_desal 
                    self.res[method] += self.dict_lcia_pb['pblcia_pem' + db.split('remind')[1]][index] * n_fc 
    ...
    def print_results(self):
        ...
        print('backup H2 consumed:'.ljust(50) + '%.3f MWh' % (self.res['h2_needed']/1e6) )
        ...
        print('cost due to LIQ:'.ljust(30) + '%.1f %%' % (self.res['cost_frac_liq']*1e2))
        ...
        print('cost due to backup:'.ljust(30) + '%.1f %%' % (self.res['cost_frac_backup']*1e2))
        ...
        for method in self.pb_names[:1]:
            ...
            print(method + ' due to LIQ:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_h2liq' %method]*1e2))
            ...
            print(method + ' due to boat:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_ship' %method]*1e2))
            print(method + ' due to refr on boat:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_refr' %method]*1e2))
            print(method + ' due to backup:'.ljust(50) + '%.5f %%' % (self.res['%s_frac_backup' %method]*1e2 ))
            print('')
        ...
        print('size LIQ:'.ljust(30) + '%.2f MW' % ((self.par['e_liq'] * self.par['n_liq'] * 1e3) /1e6))
        print('size LIQ:'.ljust(30) + '%.2f tonne/day' % (self.par['n_liq']/1e3*24.))
        ...
        print('size FC:'.ljust(30) + '%.2f MW' % (self.par['n_fc']/1e6))
        print('number of ships:'.ljust(30) + '%.2f' % (self.n_ship))
        print('size LIQ on boat:'.ljust(30) + '%.2f MW' % (self.par['e_liq'] * self.refr_kg_h * 1e3 /1e6 ))
        print('size LIQ on boat:'.ljust(30) + '%.2f tonne/day' % (self.refr_kg_h/1e3*24))

        self.e_bat = np.roll(self.e_bat,-int(self.par['start']))
        self.m_h2 = np.roll(self.m_h2,-int(self.par['start']))
        self.p_to_pemel = np.roll(self.p_to_pemel,-int(self.par['start']))

        plt.plot(self.e_bat/self.par['n_bat'],'-')
        plt.ylabel('SOC battery')
        plt.show()

        plt.plot(self.m_h2/self.par['n_tank_h2'],'-')
        plt.ylabel('SOC H2 tank')
        plt.show()

        plt.plot(self.p_to_pemel/self.par['n_pemel'],'-')
        plt.ylabel('p to pemel')
        plt.show()
