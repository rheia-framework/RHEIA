...
class Evaluation:
    ...
    def cost(self):
        ...
        time_trip = self.par['trip_length'] * 1.852 / speed #h
        ...
        boat_cap_m3 = boat_cap_twh * 1e6 * 3600. / 18.8 / 680. #m3

        boat_cap_tonne = boat_cap_twh * 1e6 * 3600. / 18.8 / 1e3 #tonne
        ...
        self.fuel_needed_kg = fuel_needed / (5.2e-3) * self.par['life_sys'] #kg ammonia needed for boat in lifetime
        
        bog_fuel = (1. - (1.-self.par['bog'])**(time_trip/24.)) * nh3_produced #MWh boil-off per year. only 1 part of the trip, het naartoe gaan, is considered as fuel

        self.mwh_produced = nh3_produced - fuel_needed
        ...
        if fuel_needed < bog_fuel:
            ...
            refr_kg_year = refr_kg_h * time_trip * times_op_en_af
            
            self.mwh_produced -= refr_kg_year * self.par['e_liq'] / (18.8/3.6*0.44) / 1e3
        else:
            refr_cost = 0.
        ...
        extra_backup = cost / (1. - self.res['nh3_needed']/1e12 - self.fuel_needed/1e6) - cost
        cost += extra_backup
    ...
    def calc_lca(self):
        ...
        for index,method in enumerate(self.pb_names):
            ...
            extra_backup = self.res[method] / (1. - self.res['nh3_needed']/1e12 - self.fuel_needed/1e6) - self.res[method]
            ...
            self.res['%s_frac_backup' %method] = extra_backup / self.res[method] 
        ...
        if self.all_dbs:
            for db in self.dbs:
                ...
                for index,method in enumerate(self.pb_names):
                    ...
                    self.res[method] += self.dict_lcia_pb['pblcia_nh3_comb' + db.split('remind')[1]][index] * (self.fuel_needed_kg + self.m_nh3_for_backup)
    ...
    def print_results(self):
        ...
        print('cost due to boat:'.ljust(50) + '%.1f %%' % (self.res['cost_frac_boat']*1e2))
        ...
        for method in self.pb_names[:1]:
            ...
            print(method + ' due to fuel boat:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_nh3_comb' %method]*1e2))
            print(method + ' due to cracking:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_cracking' %method]*1e2))
            print(method + ' due to backup:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_backup' %method]*1e2))
            print('')
            
        print('size wind:'.ljust(50) + '%.2f MW' % (self.par['n_wind']/1e6))
        print('size PV:'.ljust(50) + '%.2f MW' % (self.par['n_pv']/1e6))
        print('size PEM:'.ljust(50) + '%.2f MW' % (self.par['n_pemel']/1e6))
        print('size BAT:'.ljust(50) + '%.2f MWh' % (self.par['n_bat']/1e6))
        print('life BAT:'.ljust(50) + '%.2f year' % (self.lifeBat))
        print('size ASU:'.ljust(50) + '%.2f MW' % (self.par['n_asu']/1e6))
        print('size ASU:'.ljust(50) + '%.2f t/year' % (self.par['n_asu']/1e3 / self.par['e_asu'] * 8.760 ))
        print('size Haber-Bosch:'.ljust(50) + '%.2f MW' % (self.par['n_hb']/1e6))
        print('size desalination:'.ljust(50) + '%.2f MW' % (self.res['n_desal']/1e6))
        print('size desalination:'.ljust(50) + '%.2f m3/d' % (self.res['n_desal']/1e6 * 24. / self.par['e_desal']))
        print('size H2 tank:'.ljust(30) + '%.2f tonne (%.2f GWh)' % (self.par['n_tank_h2']/1e3, self.par['n_tank_h2']*33.3/1e6))
        print('size compressors:'.ljust(50) + '%.2f MW' % (self.par['n_compressor']/1e6))
        print('size turbine:'.ljust(50) + '%.2f MW' % (self.par['n_nh3_turb']/1e6))
