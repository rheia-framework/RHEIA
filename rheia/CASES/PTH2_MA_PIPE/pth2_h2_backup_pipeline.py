...
class Evaluation:
    ...
    def cost(self):
        ...
        losses_fill_compressor = 0.021 # 2.1%/1000km
        
        losses_booster_compressor = 0.021 * self.par['pipeline_length']/1e3

        h2_produced *= (1. - losses_fill_compressor)

        h2_produced *= (1. - losses_booster_compressor)

        self.losses_fill_compressor = losses_fill_compressor
        self.losses_booster_compressor = losses_fill_compressor

        self.par['n_fill_compressor'] = self.mw_h2 #MW H2
        
        self.par['n_booster_compressor'] = self.mw_h2 * (1. - losses_fill_compressor) #this is the H2 that passes through the lines
        self.par['n_pipeline'] = self.mw_h2 * (1. - losses_fill_compressor) * self.par['pipeline_length'] #this is the H2 that passes through the lines

        # eur/MW (H2)
        fill_compressor_cost = self.par['n_fill_compressor'] * (self.par['capex_fill_compressor'] *
                                            (crf + self.par['opex_fill_compressor']))

        components_cost += fill_compressor_cost

        booster_compressor_cost = self.par['n_booster_compressor'] * (self.par['capex_booster_compressor'] *
                                            (crf + self.par['opex_booster_compressor']))

        components_cost += booster_compressor_cost
        ...
        self.mwh_produced = h2_produced

        if self.res['h2_needed']/1e12 > 0.10:
            self.final_check = False

        # the levelized cost of hydrogen
        cost =  components_cost
        extra_backup = cost / (1. - self.res['h2_needed']/1e12) - cost
        cost += extra_backup
        ...
        self.res['cost_frac_pipe'] = (pipeline_cost + booster_compressor_cost + fill_compressor_cost) / cost + (extra_losses_booster_compressor + extra_losses_fill_compressor) / cost
    ...
    def calc_lca(self):
        ...
        n_pipe = self.par['pipeline_length'] / (self.par['life_pipeline']/self.par['life_sys']) * self.mw_h2 * (1. - 0.021)  / (110e15/1e6/(8760.*3600.)) / 0.694 #0.694 is cap decrease compared to CH4
        ...
        for index,method in enumerate(self.pb_names):
            ...
            extra_backup = self.res[method] / (1. - self.res['h2_needed']/1e12) - self.res[method]
        ...
        if self.all_dbs:
            ...
            for db in self.dbs:
                ...
                for index,method in enumerate(self.pb_names):
                    ...
                    self.res[method] += self.dict_lcia_pb['pblcia_ch4_pipeline' + db.split('remind')[1]][index] * n_pipe 
    ...
    def print_results(self):
        ...
        print('cost due to pipeline and compressor:'.ljust(30) + '%.1f %%' % (self.res['cost_frac_pipe']*1e2))
        ...
        for method in self.pb_names[:1]:
            ...
            print(method + ' due to pipe:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_pipe' %method]*1e2))
