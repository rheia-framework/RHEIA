...
class Evaluation:
    ...
    def cost(self):
        ...
        losses_fill_compressor = 0.008 # 3%/1000km        
        
        self.losses_fill_compressor = losses_fill_compressor

        losses_booster_compressor = 0.008 * self.par['pipeline_length']/1e3
        self.losses_booster_compressor = losses_booster_compressor

        ch4_produced *= (1. - losses_fill_compressor)
        
        ch4_produced *= (1. - losses_booster_compressor)
        
        self.par['n_fill_compressor'] = self.mw_ch4 #MW H2
        
        self.par['n_booster_compressor'] = self.mw_ch4 * (1. - losses_fill_compressor) #this is the H2 that passes through the lines
        self.par['n_pipeline'] = self.mw_ch4 * (1. - losses_fill_compressor) * self.par['pipeline_length'] #this is the H2 that passes through the lines
        ...
        self.mwh_produced = ch4_produced
        
        self.m_ch4_boiled_off_for_fuel = 0.
        ...
        extra_backup = cost / (1. - self.res['ch4_needed']/1e12) - cost
        cost += extra_backup
    ...
    def calc_lca(self):
        ...
        n_pipe = self.par['pipeline_length'] / (self.par['life_pipeline']/self.par['life_sys']) * self.mw_ch4 * (1. - 0.03) / (110e15/1e6/(8760.*3600.))            
        m_ch4_produced = self.mwh_produced/(50./3600.) * self.par['life_sys'] 
        ...
        for index,method in enumerate(self.pb_names):
            ...
            self.res[method] += self.dict_lcia_pb['pblcia_ng_comb_real_SSP1-Base_reference'][index] * (self.m_ch4_boiled_off_for_fuel + self.m_ch4_for_backup) #+ m_ch4_produced * (self.par['e_ratio_ng_h2'] - 1.)/self.par['e_ratio_ng_h2']
            extra_backup = self.res[method] / (1. - self.res['ch4_needed']/1e12) - self.res[method]
            ...
            self.res['%s_frac_pipeline' %method] = self.dict_lcia_pb['pblcia_ch4_pipeline_SSP1-Base_reference'][index] * n_pipe / self.res[method] + (extra_losses_booster_compressor + extra_losses_fill_compressor) / self.res[method]
        ...
        if self.all_dbs:
            ...
            for db in self.dbs:
                ...
                for index,method in enumerate(self.pb_names):
                    ...
                    self.res[method] += self.dict_lcia_pb['pblcia_ng_comb_real' + db.split('remind')[1]][index] * (self.m_ch4_boiled_off_for_fuel + self.m_ch4_for_backup)
    ...
    def print_results(self):
        ...
        print('cost due to pipeline:'.ljust(30) + '%.1f %%' % (self.res['cost_frac_pipe']*1e2))
        ...
        for method in self.pb_names[:1]:
            ...
            print(method + ' due to pipeline:'.ljust(50) + '%.1f %%' % (self.res['%s_frac_pipeline' %method]*1e2))
